...
class MapSquare:
    ...
    def generateMap(self):
        ...
        if self.load != False:
            ...
            for i in things:
                for j in things[i]:
                    if(i == 'Base' or i == 'Vitimas' or i == 'Parede'):
                        pos = j.split(",")
                        ## Define que naquela posicao vai ter determinado objeto
                        self.listPlaces[int(pos[1])][int(pos[0])].itemInside = i
                        ## Atualiza a cor do lugar
                        self.listPlaces[int(pos[1])][int(pos[0])].updateColor()
